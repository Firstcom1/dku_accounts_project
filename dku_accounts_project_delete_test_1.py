...
Databasename = "accounts_delete_6.db"
...
class MainView(QMainWindow):
    ...
    def setupUI(self):
        ...
        periodStartDay_detail = str(periodStartDay.year()) + "-" + str(periodStartDay.month()) + "-" + str(
            periodStartDay.day())
        ...
        addItem_dateMoney = str(addItem_date.year()) + "-" + str(addItem_date.month()) + "-" + str(
            addItem_date.day())  # Default값: "2021-12-01"
        ...
        UI_set.BT_addItem.clicked.connect(insertData)

        UI_set.TW_displayAllAccounts.cellDoubleClicked.connect(DeleteData)
        self.setTable()
    ...
    def toDataFile(self):
        ...
        data = {'mtype': addItem_typeMoney, 'date': addItem_dateMoney, 'category': addItem_categoryMoney,
    ...
    def cmpUserStat(self):
        ...
        df2.rename({'01.식료품 · 비주류음료 (원)': '음식', '02.주류 · 담배 (원)': '음식', '03.의류 · 신발 (원)': '취미', '04.주거 · 수도 · 광열 (원)':
            '생활', '05.가정용품 · 가사서비스 (원)': '생활', '06.보건 (원)': '생활', '07.교통 (원)': '생활', '08.통신 (원)': '생활',
                    '09.오락 · 문화 (원)': '취미', '10.교육 (원)': '공부', '11.음식 · 숙박 (원)': '음식', '12.기타상품 · 서비스 (원)': '기타'},
                   inplace=True)
    ...
    def setTable(self):
        # Table 가로(column) 갯수
        UI_set.TW_displayAllAccounts.setColumnCount(8)


        # Table 읽기전용
        UI_set.TW_displayAllAccounts.setEditTriggers(QAbstractItemView.NoEditTriggers)

        # Table 칼럼 헤더 라벨
        UI_set.TW_displayAllAccounts.setHorizontalHeaderLabels(['', '현금흐름 유형', '날짜', '카테고리', '사용처', '금액', '코멘트', ''])
...
def CreateTable():
        ...
        if not rows:
            sql = "CREATE TABLE aaa (idx INTEGER PRIMARY KEY,typeMoney TEXT, dateMoney TEXT, categoryMoney TEXT, placeMoney TEXT, amountMoeny INTEGER,commentMoney TEXT)"
            cur.execute(sql)
            conn.commit()
...
def insertData():
    typeMoney = UI_set.CB_typeMoney.currentText()
    dateMoney = UI_set.DE_dateMoney.text()
    categoryMoney = UI_set.CB_categoryMoney.currentText()
    placeMoney = UI_set.LE_placeMoney.text()
    amountMoeny = UI_set.LE_amountMoeny.text()
    commentMoney = UI_set.LE_commentMoney.text()
    conn = sq.connect(Databasename)
    cur = conn.cursor()
    sql = "INSERT INTO aaa (typeMoney, dateMoney, categoryMoney, placeMoney, amountMoeny, commentMoney) VALUES (?,?,?,?,?,?)"
    cur.execute(sql, (typeMoney, dateMoney, categoryMoney, placeMoney, amountMoeny, commentMoney))
    conn.commit()
...
def DeleteData(row, column):
    ...
    if column == 7:
        ...
        idx = UI_set.TW_displayAllAccounts.item(row, 0).text()
...
def setTables(row):
    ...
    # 갯수만큼 테이블의 Row를 생성한다.
    UI_set.TW_displayAllAccounts.setRowCount(count)

    # row 리스트만큼 반복하며 Table에 DB 값을 넣는다.
    for x in range(count):
        # 리스트 내부의 column쌍은 튜플로 반환하므로 튜플의 각 값을 변수에 저장
        idx, typeMoney, dateMoney, categoryMoney, placeMoney, amountMoeny, commentMoney = row[x]

        # 테이블의 각 셀에 값 입력
        UI_set.TW_displayAllAccounts.setItem(x, 0, QTableWidgetItem(str(idx)))
        UI_set.TW_displayAllAccounts.setItem(x, 1, QTableWidgetItem(typeMoney))
        UI_set.TW_displayAllAccounts.setItem(x, 2, QTableWidgetItem(dateMoney))
        UI_set.TW_displayAllAccounts.setItem(x, 3, QTableWidgetItem(categoryMoney))
        UI_set.TW_displayAllAccounts.setItem(x, 4, QTableWidgetItem(placeMoney))
        UI_set.TW_displayAllAccounts.setItem(x, 5, QTableWidgetItem(str(amountMoeny)))
        UI_set.TW_displayAllAccounts.setItem(x, 6, QTableWidgetItem(commentMoney))
        UI_set.TW_displayAllAccounts.setItem(x, 7, QTableWidgetItem("삭제"))
