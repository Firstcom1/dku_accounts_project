...
from PySide2.QtWidgets import QApplication, QMainWindow, QTableWidget, QTableWidgetItem, QGraphicsView, QGraphicsScene, QAbstractItemView
...
Databasename = "accounts_delete_2.db"
...
class MainView(QMainWindow):
    ...
    def setupUI(self):
        ...
        UI_set.TW_displayExpenditure.cellDoubleClicked.connect(DeleteData)
    ...
    def setTable(self):
     # Table 가로(column) 갯수
     UI_set.TW_displayExpenditure.setColumnCount(8)

     # Table 읽기전용
     UI_set.TW_displayExpenditure.setEditTriggers(QAbstractItemView.NoEditTriggers)

     # Table 칼럼 헤더 라벨
     UI_set.TW_displayExpenditure.setHorizontalHeaderLabels(['', '현금흐름 유형', '날짜', '카테고리', '사용처', '금액', '코멘트',''])
...
def CreateTable():
    ...
    if not rows:
        sql = "CREATE TABLE aaa (idx INTEGER PRIMARY KEY, placeMoney TEXT, amountMoeny INTEGER, commentMoney TEXT)"
        cur.execute(sql)
        conn.commit()
...
def insertData():
    ...
    sql = "INSERT INTO aaa (  placeMoney, amountMoeny, commentMoney) VALUES (?,?,?)"
    cur.execute(sql, ( placeMoney, amountMoeny, commentMoney))
    conn.commit()
...
def DeleteData(row, column):
    ...
    if column == 7:
        ...
        idx = UI_set.TW_displayExpenditure.item(row, 0).text()
...
def setTables(row):
    ...
    # 갯수만큼 테이블의 Row를 생성한다.
    UI_set.TW_displayExpenditure.setRowCount(count)

    # row 리스트만큼 반복하며 Table에 DB 값을 넣는다.
    for x in range(count):
        # 리스트 내부의 column쌍은 튜플로 반환하므로 튜플의 각 값을 변수에 저장
        idx, placeMoney, amountMoeny, commentMoney = row[x]

        # 테이블의 각 셀에 값 입력
        UI_set.TW_displayExpenditure.setItem(x, 0, QTableWidgetItem(str(idx)))

        UI_set.TW_displayExpenditure.setItem(x, 4, QTableWidgetItem(placeMoney))
        UI_set.TW_displayExpenditure.setItem(x, 5, QTableWidgetItem(str(amountMoeny)))
        UI_set.TW_displayExpenditure.setItem(x, 6, QTableWidgetItem(commentMoney))
        UI_set.TW_displayExpenditure.setItem(x, 7, QTableWidgetItem("삭제"))
